...
```{r}
DF<- read.csv(here("Data for the Code", "Facebook_data", "Facebook_2022_climate_awareness.csv"), stringsAsFactors = TRUE)
```
...
write.csv(DF_long, here("Data for the Code", "Facebook_data", "ccos_awareness.csv"), row.names = FALSE)
...
```{r}
myfiles<-list.files(here("Data for the Code", "Facebook_data"),pattern ="Facebook_2022")
...
for (i in 1:length(myfiles)){
  DF <- read.csv(here("Data for the Code", "Facebook_data", myfiles[i]))
  DF_long<- DF %>% pivot_longer(Albania:Zambia, names_to = "Country", values_to = "Number")
...
write.csv(DF_long, here("Data for the Code", "Corrected_Data_sets", my_filename), row.names = FALSE)               
}
```
...
```{r}
awareness<-read.csv(here("Data for the Code", "Corrected_Data_sets", "ccos_climate_awareness.csv"))
                    
beliefs <-read.csv(here("Data for the Code", "Corrected_Data_sets",  "ccos_Climate_beliefs.csv"))
```
Ok - that's all working. So now I can pull them all together into one big data frame.

```{r}
myfiles<-list.files(here("Data for the Code", "Corrected_Data_sets"),pattern ="ccos")

newDF<-read.csv(here("Data for the Code", "Corrected_Data_sets", myfiles[1]))

for (i in 2:length(myfiles)){
  tempDF <- read.csv(here("Data for the Code", "Corrected_Data_sets", myfiles[i]))
...
Bring in the data
```{r}
yale<- read.csv(here("Data for the Code", "Yale_International Perspective of Climate Change.csv"))
```
...
Wouldn't it be great if I could take the first row and make it a vector and then assign it as the column names? Well if I thought it, I am sure there is code for it. 

This was a pain
```{r}
as.vector(as.character((yale[1,])))
#make a vector of the character data in row one of yale
#??? not sure why this code is also needed up here, when I delted it, it did not work.???
...
Unlike the data frame above, the columns that we need to switch to not all of them, but a select few. Lets see if : works. Pleaseeeee work.


I tried it three different ways with "" and with $ and without both. None worked.

```{r}
indx <- sapply(ccs_Yale_Corrected, ccs_Yale_Corrected$Sample_2007:ccs_Yale_Corrected$Cl_aware)
emissionsfix[indx] <- lapply(emissionsfix[indx], function(x) as.numeric(as.character(x)))
```
I will have to do a faster way later. 
...
All right so Erika show me a faster way later. I am going to manually change the relevant ones to check my data out further. 
...
!!!!We will save our finished Yale csv, so make sure to re-run this when all the columns are switched to numeric that should be.!!!!!
