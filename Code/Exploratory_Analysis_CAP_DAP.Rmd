...
summary(ccos_US_Denmark)
```
Now the 3rd quartile is 29.21 and the max is 88.19. this may be because the United States is larger than Denmark, skewing the means, so lets use graphs instead.

```{r}
ccos_US_Denmark_worry<- ccos_US_Denmark %>% filter(ccos_US_Denmark$question=="Ques__climate_worry")
ggplot(ccos_US_Denmark, aes(x="response", y="Number")) + geom_bar(stat='identity', position='dodge', colour='Country')
```




To check for outliers I do not think we can graph all of them
```{r}
ggplot(ccos_US_Denmark, aes(x=Question))
```
...
```{r}
allfiles<- list.files(here("Data for the Code", "Facebook_data"), pattern="Facebook_2022")
```

```{r}
pivot_longer(Agentina:Vietnam, names_to="Country", values_to="Number")
```
